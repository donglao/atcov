...
class AttentionConv2D(torch.nn.Module):
    ...
    def create_positional_encoding(self):
        ...
        return pe[:seq_length].permute(1,0).to(device)
    ...
    def forward(self, x):
        ...
        # 'pos_emb' has shape [input_size, neighbor_size]
        
        x_expanded = self.expand_shift(x) + pos_emb[None,:,None,None,: ]
        # 'x_expanded' has shape [batch, input_size, h, w, neighbor_size]
        
        K = self.to_k(self.layernorm(x_expanded.permute(0,4,2,3,1)))
